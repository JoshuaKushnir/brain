# Luminance

Luminance is a regenerating player resource representing the soul's resistance to unfamiliar [[Darkness]]. It is not mana, faith, visibility, or generic life energy.

A character has current Luminance, maximum Luminance, regeneration, and [[Dark Tolerance]]. Luminance regenerates when conditions fall within what the soul is accustomed to. It drains when the character plunges into Darkness deeper than that accustomed range.

Conceptually:

`drain = max(0, local darkness - dark tolerance) × exposure rate`

Recovery is supported by safety, rest, Dawn Anchors, companionship, fulfilled Oaths, and [[Crownlight]]. Low Luminance causes distortion, false paths, identity pressure, and dangerous [[Death]]. At zero, Darkness begins directly proposing changes to the character.

Going Downed does not itself spend Luminance. Ambient exposure continues while the player is helpless.
